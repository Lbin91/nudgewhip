...
//
// 이벤트(NudgeWhipRuntimeEvent)에 따라 상태 스냅샷을 갱신하는 불변 리듀서와
...
enum NudgeWhipRuntimeState: String, Codable, CaseIterable, Sendable {
    case limitedNoAX
    case monitoring
    case pausedManual
...
enum NudgeWhipContentState: String, Codable, CaseIterable, Sendable {
    case focus = "Focus"
...
enum NudgeWhipRuntimeEvent: Equatable, Sendable {
...
struct RuntimeSnapshot: Equatable, Sendable {
    var runtimeState: NudgeWhipRuntimeState = .limitedNoAX
    var contentState: NudgeWhipContentState = .focus
...
struct RuntimeTransitionLogEntry: Equatable, Sendable {
    let event: NudgeWhipRuntimeEvent
...
enum RuntimeStateReducer {
    ...
    static func reduce(_ snapshot: RuntimeSnapshot, event: NudgeWhipRuntimeEvent, at date: Date) -> RuntimeSnapshot {
        var next = snapshot
    ...
    private static func resolveBaseRuntimeState(from snapshot: RuntimeSnapshot) -> NudgeWhipRuntimeState {
    ...
    private static func baseContentState(for runtimeState: NudgeWhipRuntimeState) -> NudgeWhipContentState {
        switch runtimeState {
