...
| 항목 | 내용 |
|------|------|
| 동작 | TTS 토글이 On일 때만 표시. `NSSpeechSynthesizer` 또는 미리 녹음된 오디오로 "다시 집중해볼까요?" 1회 재생 |
| 제약 | 프리뷰는 1회만 재생. 반복 재생 버튼 별도 제공 |
| TTS Off 시 | 음성 프리뷰 영역 숨김 또는 "음성 알림을 켜면 미리 들을 수 있습니다" 회색 표시 |

**TTS 미구현 상태 고려:**
- 현재 TTS 음성 합성이 구현되지 않았으므로 **1차 구현에서는 시스템 사운드만** 프리뷰
- 시스템 사운드(Tink/Hero/Sosumi)는 이미 구현되어 있으므로, 각각의 사운드를 버튼 클릭으로 재생
- TTS 구현 완료 후 음성 프리뷰로 교체
...
| TTS 음성 프리뷰 | AVSpeechSynthesizer로 한국어/영어 음성 재생 |
...
| 파일 | 역할 |
|------|------|
| `nudge/Onboarding/Views/NudgePreviewCard.swift` | 프리뷰 섹션 전체 뷰. 3개 프리뷰 버튼 + 사운드 재생 + 타임라인 |
| `nudge/Onboarding/Views/NudgePreviewOverlay.swift` | 인라인 오버레이 애니메이션 뷰. AlertOverlayView 로직 재사용 |

### 4.2 기존 파일 수정

| 파일 | 변경 |
|------|------|
| `BasicSetupStepView.swift` | 하단에 `NudgePreviewCard` 추가 |
...
```swift
// NudgePreviewOverlay.swift - 개념 코드
...
| 단계 | 작업 | 산출물 |
|------|------|--------|
| Phase 1 | NudgePreviewCard + NudgePreviewOverlay 구현 | 인라인 프리뷰 3종 + 사운드 |
