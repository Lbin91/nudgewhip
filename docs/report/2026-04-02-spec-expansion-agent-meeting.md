...
- 기본 알림 흐름은 `무입력 임계 도달 -> 1차 perimeter pulse -> 45~60초 추가 무입력 시 강한 시각 넛지 -> 60~90초 추가 지속 시 3차 시스템 알림 -> 장기 미복귀 시 iOS RemoteEscalation`으로 둔다.
- `Grayscale`은 기본 경험이 아니라 고강도 실험 옵션으로 분리한다.
- 3차 시스템 알림은 짧은 1회성 문구, locale 자동 매칭, 중복 발행 금지, 입력 복귀 시 즉시 해제 규칙을 따른다.
- 피로도 방지를 위해 `시간당 최대 알림 횟수`, `3차 알림 최대 횟수`, `복귀 후 쿨다운`, `같은 대사 재사용 금지 창`, `반복 오경보 시 break 제안`을 PRD에 넣는다.
...
- 테스트용 의존성 주입 포인트는 `Clock`, `EventMonitor`, `PermissionProvider`, `FrontmostAppProvider`, `NotificationCenterClient`, `CloudKitClient`를 기본 세트로 정의한다.
