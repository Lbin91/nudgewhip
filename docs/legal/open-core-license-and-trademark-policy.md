...
- 위 항목 중 "프리미엄 카피, 가격 실험, 전환 퍼널 세부 데이터"는 `docs/app/spec.md` Section 14에 명시되지 않았으나, 운영상 민감 정보로 분류하여 추가 포함한다. spec과의 범위 차이는 출시 전 검토에서 정렬한다.
...
- 외부 기여물은 공개 코드에 적용된 라이선스(Apache-2.0 또는 최종 확정 라이선스)와 동일한 조건으로 제출된 것으로 간주한다.
