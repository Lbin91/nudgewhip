...
| `concern` | 흐름이 오래 끊김, 담백한 확인 | strong nudge, remote escalation |
...
| `StrongNudge` | `concern` | mild | 담백한 확인 + 응원 혼합 |
| `Recovery` | `happy` | strong | 복귀 축하 |
| `Break` | `sleep` | 기본 | 휴식 모드 |
| `RemoteEscalation` | `concern` | mild | 짧은 확인 표현 |
...
| `concern` | 표정 변화 + 미세한 자세 전환 | 정적 concern 포즈 + opacity fade-in |
...
| Stage | happy | concern | cheer | sleep |
|---|---|---|---|---|
| `sprout` | 가능 | 가능 | 가능 | 가능 |
| `buddy` | 기본 | 기본 | 기본 | 기본 |
...
- Free에서 StrongNudge는 `concern` 대신 `cheer`(active)로 대체 표현한다.
- Pro 전용 감정인 `concern`의 스프라이트는 Pro 에셋 번들에 포함한다.
...
- `concern`은 사용자 비난이 아니라 흐름을 다시 확인하는 신호로만 쓴다.
- `sleep`은 휴식 모드와 야간 비활성에서만 사용한다.
- `cheer`는 과도한 축하보다 다음 행동을 유도하는 톤으로 유지한다.
- 미니멀 모드에서는 캐릭터 스프라이트 대신 추상형 시각 넛지를 사용한다.
